...
The Land of the Giants event creates one 15.24-meter (50-foot) logical humanoid through a portal. It pursues outdoor players, NPCs, and animals and emits `giantStomp` combat events with 4-heart damage; clients animate the raised foot and impact from that event.
...
| `requestTrade`, `confirmTrade`, `consumeItem` | merchant/item intent | validates proximity, categorized store stock, wallet, food/water/energy-drink effects, gasoline, and persistent current-block map discovery; God Mode purchases retain normal prices but bypass affordability and wallet deduction |
