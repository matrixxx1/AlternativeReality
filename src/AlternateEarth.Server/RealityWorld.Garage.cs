...
public sealed partial class RealityWorld
{
    ...
    private void AddHomeStations(string accountId, CanonicalEntity building, List<CanonicalEntity> furniture)
    {
        ...
        foreach(var type in new[]{"stove","garageWorkbench","weaponsBench"})
        {
            if(furniture.Any(item=>item.Properties.GetValueOrDefault("objectType")==type))continue;
            var definition=FurnitureCatalog.All.Single(item=>item.Type==type);
            var item=CreateFurnitureEntity(accountId,definition,"black","solid",true,1000+(type=="stove"?0:type=="garageWorkbench"?1:2),building.Position.Region);
