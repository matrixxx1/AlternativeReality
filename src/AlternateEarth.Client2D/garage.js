(function(root){
  ...
  function vehicles(ctx,garage,toScreen,scale,pitch){
    for(const vehicle of slots(garage)){
      ...
      else if(type==='scubaGear'){ctx.fillStyle='#d7b74f';ctx.beginPath();ctx.roundRect(-18,-20,14,34,6);ctx.roundRect(4,-20,14,34,6);ctx.fill();ctx.stroke();ctx.strokeStyle='#d7b74f';ctx.beginPath();ctx.arc(0,-6,24,Math.PI*.1,Math.PI*.9);ctx.stroke();ctx.fillStyle='#7bd0dc';ctx.beginPath();ctx.ellipse(0,-20,16,7,0,0,Math.PI*2);ctx.fill();ctx.stroke();}
