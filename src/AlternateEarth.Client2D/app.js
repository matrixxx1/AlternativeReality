(() => {
  ...
  function syncGodControls(player){if(!player)return;if(state.godTogglePending!==null&&!!player.godMode!==state.godTogglePending){ui.god.disabled=true;ui.gpsTeleport.hidden=true;ui.rebuild.disabled=true;return;}state.godTogglePending=null;ui.god.disabled=false;ui.god.checked=!!player.godMode;ui.gpsTeleport.hidden=!player.godMode;ui.rebuild.disabled=!player.godMode||state.rebuildPending;}
  ...
  function drawPlayerRanges(player){if(!player)return;drawWorldRing(player.position,6,'rgba(182,239,203,.28)',[5,5]);const weapon=player.equippedWeapon||'fist';if(weapon!=='none')drawWorldRing(player.position,weaponRanges[weapon]||0,'rgba(255,218,112,.25)',[10,7]);}
  ...
  ui.teleport.addEventListener('click',()=>{if(!state.players.get(state.playerId)?.godMode||!state.actionPoint){showToast('Wait for God Mode to finish enabling.');return;}send({type:'teleport',x:state.actionPoint.x,y:state.actionPoint.y,godMode:true});ui.actionMenu.hidden=true;});
  ...
  ui.rebuild.addEventListener('click',()=>{if(!state.players.get(state.playerId)?.godMode){showToast('Wait for God Mode to finish enabling.');return;}if(confirm('Reset and rebuild the entire server reality? Generated regions, dungeons, chests, relationships, and player-created world changes will be reset. Accounts, characters, inventories, and base ownership will remain.')){state.rebuildPending=true;ui.rebuild.disabled=true;ui.rebuild.textContent='Rebuilding world…';showToast('Rebuilding the entire reality from its geographic source…');send({type:'rebuildArea',godMode:true});}});
