(() => {
  ...
  const state = {
    ...
    pointer: { down: false, dragged: false, button: null, startX: 0, startY: 0, x: 0, y: 0 }, suppressClick: false, actionPoint: null, actionActor: null, pendingDoor: null, pendingChest: null, frame: 0, projectiles: [], outdoorFog: new Set(), areaLoading: false
  };
  ...
  addEventListener('mouseup',event=>{if(!state.pointer.down||event.button!==state.pointer.button)return;if(state.pointer.dragged&&event.button===0){state.suppressClick=true;setTimeout(()=>state.suppressClick=false,250);}if(!state.pointer.dragged&&event.button===2){state.actionPoint=toWorld({x:event.clientX,y:event.clientY});state.actionActor=nearPoint(actorsHere(),state.actionPoint,Math.max(1,16/state.scale))||null;updateActionMenu();ui.actionMenu.style.left=`${Math.min(event.clientX,innerWidth-195)}px`;ui.actionMenu.style.top=`${Math.min(event.clientY,innerHeight-240)}px`;ui.actionMenu.hidden=false;}state.pointer.down=false;state.pointer.button=null;});
