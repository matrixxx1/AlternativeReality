(() => {
  ...
  const state = {
    ...
    pointer: { down: false, dragged: false, x: 0, y: 0 }, actionPoint: null, actionActor: null, pendingDoor: null, pendingChest: null, frame: 0, projectiles: [], outdoorFog: new Set(), areaLoading: false
  };
  ...
  function updatePlayer(player, moving = false) {
    ...
    if (player.id === state.playerId) { updateMode(player.travelMode); renderEquipment(player); if(state.privateState?.inventory)renderInventory(state.privateState.inventory); }
  }
  ...
  function drawPersonShape(x,y,skin,shirt,moving,now,phase,mode='walk',facing='south'){
    ...
    if(mode==='bike'||mode==='dirtBike'||mode==='motorcycle'){
      ...
      ctx.strokeStyle=mode==='motorcycle'?'#252a2e':mode==='dirtBike'?'#e16a2e':'#d14e42';ctx.lineWidth=Math.max(2,s*.15);ctx.lineJoin='round';ctx.beginPath();ctx.moveTo(x+left,y+wheelY);ctx.lineTo(x-s*.16,y-s*.02);ctx.lineTo(x+s*.2,y+wheelY);ctx.lineTo(x+left,y+wheelY);ctx.lineTo(x+s*.36,y-s*.02);ctx.lineTo(x+right,y+wheelY);ctx.moveTo(x+s*.36,y-s*.02);ctx.lineTo(x+s*.55,y-s*.27);ctx.lineTo(x+s*.72,y-s*.25);ctx.stroke();
      if(mode!=='bike'){ctx.fillStyle=mode==='motorcycle'?'#6a7278':'#d65827';ctx.fillRect(x-s*.25,y-s*.08,s*.58,s*.42);ctx.fillStyle='#171918';ctx.fillRect(x-s*.38,y-s*.43,s*.55,s*.16);}
  ...
  function renderInventory(inventory){ui.inventory.replaceChildren();const items=inventory?.items||[];if(!items.length){ui.inventory.textContent='Empty';return;}const me=state.players.get(state.playerId);for(const item of items){const chip=document.createElement('span');chip.className='inventory-item';const label=title(item.itemType),wearable=item.itemType==='hat'||item.itemType==='magicHikingShoes'||item.itemType==='magicRunningShoes';if(item.itemType==='food'||item.itemType==='water'||item.itemType==='gallonOfGas'||wearable){const button=document.createElement('button');button.type='button';button.textContent=`${label} ×${item.quantity}`;if(wearable){const slot=item.itemType==='hat'?'hat':'shoes',equipped=item.itemType==='hat'?me?.hatOn:item.itemType==='magicHikingShoes'?me?.magicHikingShoesOn:me?.magicRunningShoesOn;button.title=equipped?'Unequip':'Equip';button.classList.toggle('equipped',!!equipped);button.addEventListener('click',()=>send({type:'setEquipment',slot,itemType:equipped?null:item.itemType}));}else{button.title=item.itemType==='gallonOfGas'?'Add to selected vehicle':'Consume';button.addEventListener('click',()=>send({type:'consumeItem',itemType:item.itemType}));}chip.append(button);}else chip.textContent=`${label} ×${item.quantity}`;ui.inventory.append(chip);}}
