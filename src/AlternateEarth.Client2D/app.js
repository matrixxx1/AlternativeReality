(() => {
  ...
  const presentActor=actor=>actor?.aiDialogueEnabled&&!actor.name?.startsWith('*')?{...actor,name:`*${actor.name}`}:actor;
  ...
  function applySnapshot(snapshot) {
    ...
    state.actors = new Map((snapshot.actors || []).map(x => [x.id, presentActor(x)]));
  ...
  function applyPrivate(privateState) {
    ...
    state.privateState = privateState; state.dungeon = privateState.dungeon ? {...privateState.dungeon,actors:(privateState.dungeon.actors||[]).map(presentActor)} : null;if(previousLocation!==state.dungeon?.id){state.path=[];state.target=null;state.followCommand=null;state.moveInFlight=false;centerOnPlayer();if(state.dungeon?.underwater)state.scale=28;}if(!state.dungeon?.isHome){ui.craftingWindow.hidden=true;state.crafting=null;}
    ...
    renderPlayerTesting(privateState.playerTesting);
  ...
  function handle(message) {
    ...
    switch (message.type) {
      ...
      case 'actorsMoved': {for(const original of message.actors){const actor=presentActor(original);if((actor.locationId||'outdoor')==='outdoor')state.actors.set(actor.id,actor);}if(state.dungeon){const local=new Map((state.dungeon.actors||[]).map(actor=>[actor.id,actor]));for(const original of message.actors){const actor=presentActor(original);if(actor.locationId===state.dungeon.id)local.set(actor.id,actor);}state.dungeon={...state.dungeon,actors:[...local.values()]};}for(const actor of message.actors)if(actor.isMoving)state.movingUntil.set(actor.id,performance.now()+700);break;}
      case 'actorsRemoved': for(const id of message.ids){state.actors.delete(id);state.relationships.delete(id);state.abductions.delete(id);state.burningCharacters.delete(id);state.movingUntil.delete(id);}if(state.dungeon)state.dungeon={...state.dungeon,actors:(state.dungeon.actors||[]).filter(a=>!message.ids.includes(a.id))};if(message.ids.includes(state.followCommand?.targetId))stopTravel('Target left.');if(message.ids.includes(state.npcDialogue?.actorId))closeNpcDialogue('That character is no longer here.');break;
      ...
      case 'npcDialoguePending': if(state.npcDialogue?.actorId===message.actorId){state.npcDialogue.pending=true;ui.npcDialogueStatus.textContent='Waiting for LocalAI…';ui.npcDialogueInput.disabled=true;ui.npcDialogueSend.disabled=true;}break;
      case 'npcDialogueResult': receiveNpcDialogue(message);break;
      case 'npcDialogueError': failNpcDialogue(message);break;
      ...
      case 'actorRemoved': state.actors.delete(message.actorId);if(state.dungeon)state.dungeon={...state.dungeon,actors:(state.dungeon.actors||[]).filter(actor=>actor.id!==message.actorId)};if(message.actorId===state.npcDialogue?.actorId)closeNpcDialogue('That character is no longer here.');break;
      ...
      case 'playerTestingUpdated': applyPrivate(message.privateState);if(message.privateState?.playerTesting?.cantTeleport&&state.godPlacement==='teleport')cancelGodPlacement();if(!ui.actionMenu.hidden)updateActionMenu();state.lastMiniMapDraw=-Infinity;showToast(message.message||'Player testing rules saved.');break;
      ...
      case 'inversionsUpdated': state.inversions=message.inversions;syncGodVoteControls();if(message.progression&&state.privateState){state.privateState.progression=message.progression;renderProgression();}Inversions.tick();break;
      ...
      case 'combatEvent': if(state.npcDialogue&&[message.combat?.attackerId,message.combat?.targetId].includes(state.npcDialogue.actorId)&&[message.combat?.attackerId,message.combat?.targetId].includes(state.playerId))closeNpcDialogue('Conversation ended by combat.');receiveCombat(message.combat);receiveCombatFear(message.combat);break;
  ...
  function updatePlayer(player, moving = false) {
    ...
    if(player.id===state.playerId&&player.survival?.illnesses?.some(i=>i.chargeRemainingMeters>0)){state.path=[];state.target=null;state.keys.clear();state.followCommand=null;}
  ...
  function syncGodControls(player){if(!player)return;ui.serverConfigButton.disabled=false;ui.serverConfigButton.title='Server configuration';syncGodConfigTab(player);ui.rebuild.disabled=state.rebuildPending;if(!ui.actionMenu.hidden)updateActionMenu();}
  ...
  function drawFurnishing(item,now,preview=false){const p=toScreen(item.position),type=item.properties?.objectType||'chair',size=furnitureSize(item),w=Math.max(8,size.w*state.scale),d=Math.max(6,size.d*state.scale*state.pitch),color=furnitureColors[item.properties?.color]||'#785538',pattern=item.properties?.pattern||'solid';ctx.save();ctx.globalAlpha=preview?.62:1;ctx.translate(p.x,p.y);ctx.fillStyle=color;ctx.strokeStyle='#30251d';ctx.lineWidth=2;
    ...
    else if(type==='sewingTable'){ctx.fillStyle='#8c6845';ctx.fillRect(-w/2,-d/2,w,d);ctx.strokeRect(-w/2,-d/2,w,d);ctx.fillStyle='#d8d2c2';ctx.fillRect(-w*.2,-d*.28,w*.38,d*.5);ctx.strokeStyle='#2f3438';ctx.lineWidth=2;ctx.beginPath();ctx.arc(0,-d*.06,Math.min(w,d)*.18,0,Math.PI*2);ctx.stroke();ctx.beginPath();ctx.moveTo(w*.18,-d*.28);ctx.lineTo(w*.3,d*.28);ctx.stroke();ctx.fillStyle='#b53838';ctx.beginPath();ctx.arc(-w*.3,d*.2,Math.min(w,d)*.11,0,Math.PI*2);ctx.fill();}
  ...
  const travelVehicleTypes=new Set(['skateboard','bike','eBike','dirtBike','motorcycle','inflatableRaft','scuba','ufo']);
  const vehicleTypes=new Set(['skateboard','bike','eBike','dirtBike','motorcycle','inflatableRaft','scubaGear','ufo','gallonOfGas']);
  ...
  Object.assign(itemGlyphs,{dirtyWater:'🟤',purifiedWater:'💧',stove:'♨',sewingTable:'🧵',garageWorkbench:'🛠',weaponsBench:'⚒',waterFilter:'▧',craftingTable:'🛠',emptyGlassBottle:'🍾',emptyPlasticBottle:'🧴',crustySocks:'🧦',soiledUnderwear:'🩲',paper:'📄',cloth:'▧',thread:'🧵',needle:'🪡',leather:'▰',plastic:'♳',styrofoam:'▤',drugs:'💊',charcoal:'◼',potassiumNitrate:'⚗',salt:'🧂',pepper:'🧂',sulfur:'🟡',bleach:'🧴',ammonia:'🧴',sulfuricAcid:'⚗',cookingOil:'🫙',flour:'▣',sugar:'▣',cookingSupplies:'🍳',craftingGas:'⛽',gunpowder:'◉','recipe:gunpowder':'📜','recipe:molotovCocktail':'📜'});
  ...
  function playerTestingRule(key){return state.privateState?.playerTesting?.[key]!==false;}
  function effectiveCarryingCapacity(me,fallback=50){if(!playerTestingRule('obeysBackpackWeightLimit')||me?.godMode)return Infinity;const phase=energyDrinkPhase(me),base=Number(state.privateState?.progression?.carryingCapacity)||Number(fallback)||50;return phase?.phase==='boost'?base*2:phase?.phase==='crash'?base*.2:base;}
  function updateBackpackSummary(inventory,me){if(!inventory)return;const weight=Number(inventory.weightPounds)||0,maximumWeight=effectiveCarryingCapacity(me,inventory.maximumWeightPounds),unlimited=!Number.isFinite(maximumWeight),penalty=unlimited?0:Math.min(50,weight);ui.backpackSummary.textContent=`Wallet $${((me?.walletCents||0)/100).toFixed(2)} · ${weightText(weight)} / ${unlimited?'Unlimited':maximumWeight.toFixed(0)+' lb'}${unlimited?' · no weight speed penalty':penalty>0?` · ${penalty.toFixed(0)}% slower`:''}${me?.travelMode==='eBike'?` · E-bike battery ${Math.max(0,Number(me.eBikeRemainingMeters||0)/1609.344*100).toFixed(0)}%`:me?.travelMode==='dirtBike'?` · Dirt bike ${Number(me.dirtBikeGasGallons||0).toFixed(2)} gal`:me?.travelMode==='motorcycle'?` · Motorcycle ${Number(me.motorcycleGasGallons||0).toFixed(2)} gal`:me?.travelMode==='ufo'?` · UFO fuel ${(Number(me.ufoRemainingMeters||0)/1609.344).toFixed(2)} mi loaded`:''}`;ui.backpackSummary.classList.toggle('over-capacity',weight>maximumWeight+.001);}
  ...
  function openServerConfigWindow(){
    syncGodConfigTab(state.players.get(state.playerId));setServerConfigTab('world');
  ...
  async function refreshRealityInfo(){
    ...
    try{
      const response=await fetch('/api/reality-maintenance',{cache:'no-store'});if(!response.ok)throw new Error(response.status===403?'World Testing is unavailable.':'Could not load server information.');
  ...
  function setServerConfigTab(tab){if(tab==='world')refreshRealityInfo();for(const button of ui.serverConfigWindow.querySelectorAll('[data-server-config-tab]')){const active=button.dataset.serverConfigTab===tab;button.classList.toggle('active',active);button.setAttribute('aria-selected',String(active));}for(const page of ui.serverConfigWindow.querySelectorAll('[data-server-config-page]'))page.hidden=page.dataset.serverConfigPage!==tab;}
  function renderPlayerTesting(settings){
    if(!settings)return;
    const inputs=[...ui.serverConfigWindow.querySelectorAll('[data-player-testing]')];
    for(const input of inputs)input.checked=input.dataset.playerTesting==='cantTeleport'?settings.cantTeleport===true:settings[input.dataset.playerTesting]!==false;
    const bypassed=inputs.filter(input=>!input.checked).length,status=ui.serverConfigWindow.querySelector('#playerTestingStatus');
    status.textContent=bypassed?`${bypassed} testing bypass${bypassed===1?' is':'es are'} active for this character.`:'Normal gameplay is active.';
  }
  function savePlayerTesting(){
    const settings=Object.fromEntries([...ui.serverConfigWindow.querySelectorAll('[data-player-testing]')].map(input=>[input.dataset.playerTesting,input.checked]));
    ui.serverConfigWindow.querySelector('#playerTestingStatus').textContent='Saving player testing rules…';
    send({type:'updatePlayerTesting',settings});
  }
  ...
  function refreshNpcConversations(){
    ...
    if(!ui.npcDialogueWindow.hidden&&state.npcDialogue?.actorId&&!state.npcDialogue.ended)active.add(state.npcDialogue.actorId);
  ...
  function addNpcDialogueBubble(kind,text){const bubble=document.createElement('div');bubble.className=`npc-dialogue-bubble ${kind}`;bubble.textContent=text;ui.npcDialogueMessages.append(bubble);ui.npcDialogueMessages.scrollTop=ui.npcDialogueMessages.scrollHeight;}
  function openNpcDialogue(actor){
    stopTravel();state.npcDialogue={actorId:actor.id,pending:false,ended:false,interactionId:null};ui.npcDialogueTitle.textContent=actor.name;ui.npcDialogueMessages.replaceChildren();
    const rating=state.relationships.get(actor.id)??0;ui.npcDialogueStatus.textContent=`Friend / foe: ${rating.toFixed(2)} · LocalAI`;
    ui.npcDialogueInput.disabled=false;ui.npcDialogueSend.disabled=false;showInteractionWindow(ui.npcDialogueWindow,520);refreshNpcConversations();ui.npcDialogueInput.focus();
  }
  function closeNpcDialogue(reason){
    const dialogue=state.npcDialogue;if(dialogue?.actorId)send({type:'conversationStatus',actorId:dialogue.actorId,active:false});
    if(reason)showToast(reason);ui.npcDialogueWindow.hidden=true;state.npcDialogue=null;refreshNpcConversations();
  }
  function receiveNpcDialogue(message){
    const dialogue=state.npcDialogue;if(!dialogue||dialogue.actorId!==message.actorId||dialogue.interactionId!==message.interactionId)return;
    dialogue.pending=false;dialogue.ended=message.endConversation===true;addNpcDialogueBubble('npc',message.dialogue||'…');
    if(message.relationship)state.relationships.set(message.actorId,message.relationship.friendRating);
    const rating=message.relationship?.friendRating??state.relationships.get(message.actorId)??0,source=message.usedAi?'LocalAI':'offline response';
    ui.npcDialogueStatus.textContent=`Friend / foe: ${Number(rating).toFixed(2)} · ${source}${dialogue.ended?' · conversation ended':''}`;
    ui.npcDialogueInput.disabled=dialogue.ended;ui.npcDialogueSend.disabled=dialogue.ended;if(!dialogue.ended)ui.npcDialogueInput.focus();refreshNpcConversations();
  }
  function failNpcDialogue(message){
    const dialogue=state.npcDialogue;if(!dialogue||dialogue.actorId!==message.actorId||dialogue.interactionId!==message.interactionId)return;
    dialogue.pending=false;addNpcDialogueBubble('system',message.message||'The request could not finish.');ui.npcDialogueStatus.textContent='Conversation unavailable';ui.npcDialogueInput.disabled=false;ui.npcDialogueSend.disabled=false;ui.npcDialogueInput.focus();
  }
  ...
  function openCrafting(crafting){
    $('#craftingTitle').textContent=crafting.stationType==='stove'?'Stove':crafting.stationType==='sewingTable'?'Sewing table':crafting.stationType==='garageWorkbench'?'Garage workbench':crafting.stationType==='weaponsBench'?'Weapons bench':'Crafting table';
    ...
    for(const recipe of recipes){
      ...
      const blockers=[];if(recipe.learned===false)blockers.push('Learn this recipe from dungeon loot first');if(skill.level<recipe.requiredLevel)blockers.push(`Requires crafting level ${recipe.requiredLevel.toLocaleString()}`);if(playerTestingRule('mustMeetCraftingMaterialRequirements')&&recipe.ingredients.some(ingredient=>ingredient.available<ingredient.required))blockers.push('Missing supplies (see ingredients below)');
      ...
      const detail=document.createElement('p');detail.className='crafting-effect';detail.textContent=`${(recipe.successChance*100).toFixed(2)}% success for the next batch · Studied ${recipe.studyCount} time(s) · Base ${(recipe.baseSuccessChance*100).toFixed(2)}% · Next copy +${(recipe.nextStudyBonus*100).toFixed(3)} percentage points. ${recipe.difficulty}.`;if(recipe.id.startsWith('cook')&&recipe.outputItemType.startsWith('cooked'))detail.textContent='Basic stove cooking · no recipe required · guaranteed cooked meat.';else detail.textContent+=` ${HomeWorkshop.bonusSummary(recipe.bonuses)}`;
      ...
      const warning=document.createElement('p');warning.textContent=`${playerTestingRule('canFailWhenCrafting')?'Quantities are potential output, not guaranteed successes.':'Player Testing guarantees every attempt succeeds.'} Every attempted batch earns crafting XP, doubling for each additional ingredient type.${playerTestingRule('consumesCraftingMaterials')?' Attempted batches consume materials.':' Player Testing keeps crafting materials.'}`;
  ...
  function drawMiniMap(me){
    ...
    const home=state.privateState?.base;ui.miniMapTeleportHome.disabled=!home||indoors||state.privateState?.playerTesting?.cantTeleport===true;ui.miniMapTeleportHome.title=state.privateState?.playerTesting?.cantTeleport===true?"Player can't teleport is enabled in Player Testing.":!home?'Waiting for your Home to be assigned.':indoors?'Leave the building before teleporting Home.':'Teleport directly to your Home.';plot(home?.position,'#8ff09f','⌂',6,'Home',`Home · ${home?.ownerName||me.name}'s base`,home?{targetType:'home',targetId:home.buildingId}:null);
  ...
  function requestMiniMapFastTravel(type,fastTravel){if(!fastTravel||state.privateState?.playerTesting?.cantTeleport===true)return;stopTravel();ui.miniMapTooltip.hidden=true;setWorldTask(`Teleporting to ${type}…`);send({type:'mapFastTravel',...fastTravel});}
  ...
  function syncTravelButtonState(button,player,probed=probedPhase(player)){
    ...
    const fuelStatus=range===null?'':!playerTestingRule('consumesVehicleFuel')?' Player Testing: no fuel consumption.':` Range remaining: ${range.toFixed(2)} miles.`;
...
function movementLoop(time){const me=state.players.get(state.playerId);if(me&&!controlsPaused()&&!me.abduction&&!me.ridingBusId&&!me.waitingAtBusStopId&&!isSleeping(me)&&time-state.lastInput>28&&!state.worldBusy){advanceAutoTreasure(time,me);Survival.advanceGather(state,me,send,stopTravel);gardenUI.advance(me);maintainPosturing(time,me);maintainFollowCommand(time,me);if(state.pendingMerchant){const merchant=actorsHere().find(actor=>actor.id===state.pendingMerchant.id);if(!merchant||merchant.locationId!==me.locationId)state.pendingMerchant=null;else if(Math.hypot(merchant.position.x-me.position.x,merchant.position.y-me.position.y)<=4.8){state.pendingMerchant=null;state.path=[];state.target=null;send({type:'requestTrade',merchantId:merchant.id});}}if(state.pendingDoor&&Math.hypot(state.pendingDoor.position.x-me.position.x,state.pendingDoor.position.y-me.position.y)<5.5){const door=state.pendingDoor;state.pendingDoor=null;if(doorIsLocked(door))stopTravel('That door is locked.');else{send({type:'enterDungeon',doorId:door.id});stopTravel();}}maintainTreasurePickup(me);if(state.pendingChop&&Math.hypot(state.pendingChop.position.x-me.position.x,state.pendingChop.position.y-me.position.y)<=2.3){send({type:'chopVegetation',entityId:state.pendingChop.id});state.pendingChop=null;stopTravel();}if(state.pendingPet&&Math.hypot(state.pendingPet.position.x-me.position.x,state.pendingPet.position.y-me.position.y)<=3){send({type:'captureQuestPet',actorId:state.pendingPet.id});state.pendingPet=null;stopTravel();}if(state.pendingDungeonAction&&Math.hypot(state.pendingDungeonAction.target.x-me.position.x,state.pendingDungeonAction.target.y-me.position.y)<2.7){const action=state.pendingDungeonAction.action;state.pendingDungeonAction=null;if(action==='exit')send({type:'exitDungeon'});else send({type:'changeDungeonLevel',direction:action==='up'?-1:1});stopTravel();}if(!state.moveInFlight){let dx=0,dy=0;if(state.keys.has('w')||state.keys.has('arrowup'))dy+=1;if(state.keys.has('s')||state.keys.has('arrowdown'))dy-=1;if(state.keys.has('a')||state.keys.has('arrowleft'))dx-=1;if(state.keys.has('d')||state.keys.has('arrowright'))dx+=1;if(dx||dy){state.followCommand=null;state.pendingMerchant=null;state.target=null;state.path=[];state.moveInFlight=true;send({type:'moveRequest',x:dx,y:dy,sequence:++state.pathSequence});state.lastInput=time;}else if(state.target&&(!state.areaLoading||pointInLoadedArea(state.target))){while(state.path.length&&Math.hypot(state.path[0].x-me.position.x,state.path[0].y-me.position.y)<.45)state.path.shift();const waypoint=state.path[0]||state.target,tx=waypoint.x-me.position.x,ty=waypoint.y-me.position.y,d=Math.hypot(tx,ty);if(d<(state.dungeon?.underwater ? .001 : .2)&&!state.path.length){state.target=null;}else if(d>(state.dungeon?.underwater ? .000001 : .01)){state.moveInFlight=true;send({type:'moveRequest',x:tx/d,y:ty/d,sequence:state.pathSequence,maxDistanceMeters:d,destinationX:waypoint.x,destinationY:waypoint.y});state.lastInput=time;}}}}requestAnimationFrame(movementLoop);}
  ...
  function requestTeleport(target){if(state.privateState?.playerTesting?.cantTeleport===true){showToast("Player can't teleport is enabled in Player Testing.");return;}state.followCommand=null;if(!pointInLoadedArea(target))beginAreaLoading(target,'Loading teleport destination…');else setWorldTask('Preparing teleport…');send({type:'teleport',x:target.x,y:target.y,godMode:true});}
  ...
  function updateSingleActionMenu(){
    ...
    const canTalk=actor?.aiDialogueEnabled===true,talkClose=canTalk&&!!me&&Math.hypot(me.position.x-actor.position.x,me.position.y-actor.position.y)<=6;ui.talk.hidden=!canTalk;ui.talk.disabled=!talkClose;ui.talk.textContent=actor?`Talk with ${actor.name}`:'Talk';ui.talk.title=talkClose?'Open a private LocalAI conversation.':'Move within 6 meters to talk.';
  ...
  ui.serverConfigWindow.querySelectorAll('[data-player-testing]').forEach(input=>input.addEventListener('change',savePlayerTesting));
  ...
  ui.serverConfigButton.addEventListener('click',openServerConfigWindow);
  ...
  function syncGodConfigTab(player){
    for(const button of ui.serverConfigWindow.querySelectorAll('[data-server-config-tab]'))button.disabled=false;
    for(const button of ui.serverConfigWindow.querySelectorAll('#godActionTools button'))button.disabled=button.id!=='clearTestCharactersButton'&&!!state.dungeon;
    ui.serverConfigWindow.querySelector('#godWorldTools').disabled=false;syncGodVoteControls();
  }
  function syncGodVoteControls(){
    const vote=(state.inversions||state.privateState?.inversions)?.vote;
    ui.serverConfigWindow.querySelector('#cancelServerVoteButton').disabled=!vote;
    ui.serverConfigWindow.querySelector('[data-world-event="vote"]').disabled=!!vote;
  }
  ui.serverConfigWindow.querySelector('#cancelServerVoteButton').addEventListener('click',()=>{
    if(!(state.inversions||state.privateState?.inversions)?.vote)return;
    send({type:'cancelServerVote'});
  });
  function cancelGodPlacement(){state.godPlacement=null;$('#cancelGodPlacement').hidden=true;}
  function beginGodPlacement(kind){
    if(state.dungeon||controlsPaused())return;
    stopTravel();state.godPlacement=kind;closeServerConfigWindow();ui.actionMenu.hidden=true;
    $('#cancelGodPlacement').hidden=false;$('#cancelGodPlacement').textContent=`Cancel ${kind==='teleport'?'teleport':kind+' placement'}`;
    showToast('Click a map location. Press Escape or Cancel to stop.');window.focus();
  }
  function placeGodTool(target){
    if(!state.godPlacement)return false;
    const kind=state.godPlacement;cancelGodPlacement();
    if(state.dungeon||controlsPaused())return true;
    if(kind==='teleport')requestTeleport(target);else send({type:'placeTestCharacter',kind,x:target.x,y:target.y});
    return true;
  }
  for(const button of ui.serverConfigWindow.querySelectorAll('[data-test-character]'))button.addEventListener('click',()=>beginGodPlacement(button.dataset.testCharacter));
  $('#clearTestCharactersButton').addEventListener('click',()=>send({type:'clearTestCharacters'}));
  ui.teleport.addEventListener('click',()=>{if(!state.actionPoint||state.dungeon)return;ui.actionMenu.hidden=true;requestTeleport(state.actionPoint);});
  $('#cancelGodPlacement').addEventListener('click',cancelGodPlacement);
  addEventListener('keydown',event=>{if(event.key==='Escape')cancelGodPlacement();});
  ...
  ui.talk.addEventListener('click',()=>{if(state.actionActor&&!ui.talk.disabled)openNpcDialogue(state.actionActor);ui.actionMenu.hidden=true;});
  ...
  ui.npcDialogueClose.addEventListener('click',()=>closeNpcDialogue());
  ...
  ui.npcDialogueForm.addEventListener('submit',event=>{event.preventDefault();const dialogue=state.npcDialogue,message=ui.npcDialogueInput.value.trim();if(!dialogue||dialogue.pending||dialogue.ended||!message)return;const interactionId=clientId();dialogue.interactionId=interactionId;dialogue.pending=true;addNpcDialogueBubble('player',message);ui.npcDialogueInput.value='';ui.npcDialogueInput.disabled=true;ui.npcDialogueSend.disabled=true;ui.npcDialogueStatus.textContent='Waiting for LocalAI…';send({type:'npcDialogue',actorId:dialogue.actorId,message,interactionId});});
  ...
  function confirmRealityRebuild(fromScratch) {
    return new Promise(resolve=>{
      const dialogDocument=ui.serverConfigWindow.ownerDocument||document;
      const dialog=dialogDocument.createElement('dialog');
      ...
      dialogDocument.body.append(dialog);dialog.showModal();
    });
  }
  async function requestRealityRebuild() {
    ...
    if(!confirmed)return;
    if(controlsPaused()||state.socket?.readyState!==WebSocket.OPEN){showToast('Wait for the connection to be ready, then try again.');return;}
