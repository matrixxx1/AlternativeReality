...
try{
  ...
  assert.notEqual(surfaced.player.locationId,'outdoor');assert.equal(surfaced.player.travelMode,'scuba');
  const surfaceClick=await command({type:'exitDungeon'},'dungeonExited');assert.equal(surfaceClick.player.locationId,'outdoor');assert.equal(surfaceClick.player.travelMode,'raft');
  ...
  console.log(JSON.stringify({result:'PASS',spearDelayedImpact:true,deepWaterPredators:true,surfaceMode:'raft',explicitSurfaceRequired:true,scubaAsset:true,wildFoods:true,rawMeat:true}));
  ws.close();ws=null;
