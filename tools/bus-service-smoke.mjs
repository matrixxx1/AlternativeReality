...
const root=path.resolve(import.meta.dirname,'..'),data=path.join(root,'artifacts','bus-service-smoke-'+Date.now());
...
try{
 ...
 // Wait until the idle bus has finished parking before approaching its door.
 let current=detail.buses.find(b=>b.id===bus.id);
 const parkDeadline=Date.now()+30000;
 while(current.status!=='out of service'&&Date.now()<parkDeadline){
   const start=messages.length;
   ws.send(JSON.stringify({type:'requestBusRoute',routeId:route.id,positionsOnly:true}));
   current=(await wait(m=>m.type==='busRoutePositions',start)).buses.find(b=>b.id===bus.id);
   await new Promise(r=>setTimeout(r,100));
 }
 assert.equal(current.status,'out of service');
 const dx=Math.cos(current.headingRadians),dy=Math.sin(current.headingRadians);
 mark=messages.length;ws.send(JSON.stringify({type:'teleport',x:current.position.x+dy*2,y:current.position.y-dx*2,godMode:true}));
 await wait(m=>m.type==='playerTeleported',mark);
 mark=messages.length;ws.send(JSON.stringify({type:'boardBus',busId:current.id}));
 await wait(m=>m.type==='playersUpdated'&&m.players.some(p=>p.id===id&&p.ridingBusId===current.id),mark);
 await wait(m=>m.type==='busesMoved'&&m.buses.some(b=>b.id===current.id&&b.status==='driving'&&b.speedMetersPerSecond>0),mark);
 ...
 const stopped=(await wait(m=>m.type==='busesMoved'&&m.buses.some(b=>b.id===current.id),mark)).buses.find(b=>b.id===current.id);
 mark=messages.length;ws.send(JSON.stringify({type:'setEquipment',slot:'weapon',itemType:'knife'}));
 await wait(m=>m.type==='playerUpdated'&&m.player.equippedWeapon==='knife',mark);
 mark=messages.length;ws.send(JSON.stringify({type:'teleport',x:stopped.position.x+Math.sin(stopped.headingRadians)*1.3,y:stopped.position.y-Math.cos(stopped.headingRadians)*1.3,godMode:true}));
 await wait(m=>m.type==='playerTeleported',mark);
 mark=messages.length;ws.send(JSON.stringify({type:'attack',targetId:stopped.id,weapon:'knife'}));
 const attack=await wait(m=>m.type==='combatEvent'&&m.combat.targetId===stopped.id,mark);
 assert.equal(attack.combat.weapon,'knife');
 // Server accepts the bus target and returns normal combat resolution (including possible miss).
 console.log(JSON.stringify({result:'PASS',parked:true,boarded:true,resumed:true,busCombat:true,data}));
}finally{ws.close();upstream.closeAllConnections();upstream.close();server.kill();}
