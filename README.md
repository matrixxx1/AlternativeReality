...
- Left-click anywhere in the world to walk toward that location. Left-clicking a door instead offers **Move here**, God Mode **Teleport here**, and a proximity/wallet-aware **Purchase as base** action.
...
- God Mode supplies a 5× movement multiplier, full water/stamina, a $500 wallet floor, health protection/regeneration, equipment access, whole-reality rebuilding, and teleporting. It also exposes a client-side **Hide fog** checkbox. Teleports into unloaded blocks visibly queue geographic generation and complete at the nearest safe walkable point only after that block is ready. The compass button asks the browser for location only when clicked. Browser GPS requires HTTPS or `localhost`; this prototype currently supports GPS locations within the reality's active one-degree projection region.
- God Mode also reveals a persistent server configuration editor for player base speed, base visible range, additive terrain/travel/item speed modifiers, item visibility effects, damage, range, and merchant prices. Validated changes immediately affect authoritative gameplay and survive restarts. Dynamic characters and treasure are hidden beyond visible range while geographic scenery remains visible throughout the viewport.
...
- Building doors lead to footprint-sized procedural dungeons with fog of war, enemies, chests, treasure, and an exit. A dungeon is a fresh per-player session whenever it is entered and is discarded on exit. Each account receives a permanent base assignment whose interior is always labeled **Home**, shown only to its owner with a large world-anchored roof flag and compass. Another building starts at $350,000 and increases with size, or costs $0.01 in God Mode; the server validates funds, proximity, and ownership. Home contains a fireplace and furniture; using the bed restores health, water, stamina, and both five-minute protections. Character management is available through the wardrobe inside Home.
...
- Purchased furniture is automatically placed in an open Home location when one exists. Overflow goes to **Home storage**, which is visible only inside the owner's Home. Click placed furniture for item actions: drag to move, rotate 90 degrees, or return it to storage. Beds offer **Take a nap** and wardrobes open character management. Furniture placement, variants, rotation, and storage persist in SQLite.
...
Real street names, airports, state borders/names, addresses, shops, gas stations, furniture stores, and parcel lines are imported when OpenStreetMap supplies reliable tags. Hovering a building shows its available address. Parcel boundaries become stylized fences with open gates on at least two sides. Geographic gas, clothing, food, convenience, and furniture businesses create matching merchant inventories and visually distinct storefronts. Moving any edge of the viewport into a new geographic cell starts loading before the player enters it, shades that exact area, and draws a pulsing boundary with repeated **LOADING MAP** signs and elapsed time. A persistent task banner distinguishes loading, generation, routing, and client rendering work. New terrain arrives under near-opaque fog with faint block lines; merchants sell a $1–$1,000 map that permanently reveals the current block for that player and is never offered there again. Snapshots identify exact loaded cells, so diagonal exploration cannot clear fog over an ungenerated hole inside the world's aggregate bounds. Generated maps and raw geographic caches stay local to each reality server under `data/`.

Outside God Mode, a building costs at least $350,000. Its authoritative price then grows exponentially with the footprint's estimated square footage and is displayed on the door action before purchase. God Mode continues to price any base at one cent.
