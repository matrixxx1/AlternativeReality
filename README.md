...
- God Mode supplies a 5× movement multiplier, full water/stamina, a $500 wallet floor, health protection/regeneration, equipment access, rebuilding, and teleporting. Its compass button asks the browser for location only when clicked, then teleports to the nearest safe walkable point at that real-world coordinate. Browser GPS requires HTTPS or `localhost`; this prototype currently supports GPS locations within the reality's active one-degree projection region.
