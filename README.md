...
Real street names, airports, state borders/names, addresses, shops, gas stations, and parcel lines are imported when OpenStreetMap supplies reliable tags. Hovering a building shows its available address. Parcel boundaries become stylized fences with open gates on at least two sides. Geographic gas, clothing, food, and convenience businesses create matching merchant inventories. Moving any edge of the viewport into a new geographic cell starts loading before the player enters it, shades that exact area, and draws a pulsing boundary with repeated **LOADING MAP** signs and elapsed time. A persistent task banner distinguishes loading, generation, routing, and client rendering work. New terrain arrives under near-opaque fog with faint block lines; merchants sell a $1–$1,000 map that permanently reveals the current block for that player and is never offered there again. Snapshots identify exact loaded cells, so diagonal exploration cannot clear fog over an ungenerated hole inside the world's aggregate bounds. Generated maps and raw geographic caches stay local to each reality server under `data/`.
